...
const SearchForm = () => {
  ...
  const handleIncrement = (type) => {
    ...
    if (type === 'guest' && roomGuest.guest >= maxGuestsPerRoom * roomGuest.room) {
      // If total guests reach the maximum limit (3 guests per room), don't increment.
      return;
    }
    setRoomGuest({
      ...roomGuest,
  ...
  const handleDecrement = (type) => {
    if (type === 'guest' && roomGuest[type] <= 0) {
      // If no guests or less, don't decrement.
      return;
    }
    setRoomGuest({
      ...roomGuest,
